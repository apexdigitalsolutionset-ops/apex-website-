...
// Initialize EmailJS (የራስህን Public Key እዚህ ጋር አስገባ)
...
const packagesData = {
  1: {
    ...
    am: { title: "ጥቅል 1: Ascent (መሰረቱ)", items: ["<b>የብራንድ መሰረት:</b> የሎጎ ዲዛይን፣ የብራንድ ቀለማት እና ፎንት (Typography)።", "<b>የገፅ የበላይነት:</b> የ Facebook፣ Instagram እና TikTok ገፆችን ማዋቀር እና ማስተካከል።", "<b>ስትራቴጂካዊ ይዘት:</b> በወር 10 ከፍተኛ ጥራት ያላቸው ፖስቶች።", "<b>የማህበረሰብ ተሳትፎ:</b> አድማጭዎን ንቁ ለማድረግ ኮመንት እና መልዕክቶችን ማስተዳደር።", "<b>የእድገት ምክክር:</b> የዲጂታል ስትራቴጂዎን ለማስተካከል ወርሃዊ ምክክር።"] }
  },
  2: {
    id: "Apex", priceValue: 18500,
    ...
    am: { title: "ጥቅል 2: Apex (የእድገት ማፋጠኛ)", items: ["<b>የልወጣ ኮፒራይቲንግ:</b> አሳማኝ ሁኮች፣ ታሪኮች እና ማዘዣዎች (CTAs)።", "<b>የማስታወቂያ አስተዳደር:</b> 5 ትራፊክ እና ሊድ ላይ ያተኮሩ የማስታወቂያ ዘመቻዎች።", "<b>የ Google የበላይነት:</b> የ Google Maps ሙሉ ትስስር እና ሪቪው አስተዳደር።", "<b>ዕለታዊ የበላይነት:</b> ዕለታዊ የስቶሪ እንቅስቃሴዎች እና ቋሚ ፖስቶች።", "<b>የአፈፃፀም ክትትል:</b> የተደራሽነት እና ጥራት ወርሃዊ ሪፖርቶች።", "<b>የእድገት ምክክር:</b> የስትራቴጂ ውይይቶችን ያካትታል።"] }
  },
  3: {
    id: "Zenith", priceValue: 50000,
  ...
  4: {
    ...
    am: { title: "ተጨማሪ የግል አገልግሎቶች", items: ["ለእርስዎ ፍላጎት ብቻ የሚሆኑ የተነጠሉ አገልግሎቶችን ይምረጡ። የተወሰነውን ዋጋ ለማየት እና ለመክፈል ከታች ካሉት አማራጮች ውስጥ አንዱን ይጫኑ።"] }
  }
};

const addOnServices = [
  { id: 'logo', nameEn: 'Professional Logo Design', nameAm: 'ፕሮፌሽናል የሎጎ ዲዛይን', price: 1500, days: 5 },
  { id: 'bot', nameEn: 'Custom Telegram Bot', nameAm: 'የቴሌግራም ቦት ማበልፀግ', price: 6000, days: 15 },
  { id: 'web', nameEn: 'Premium Website Design', nameAm: 'ፕሪሚየም ዌብሰይት ዲዛይን', price: 16000, days: 20 },
...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      alert(currentLang === 'en' ? "User not found. Please register." : "ተጠቃሚው አልተገኘም። እባክዎ ይመዝገቡ።");
    }
  } catch (err) {
    console.error(err);
  }
...
// Verification Email Sender (EmailJS)
let expectedVerifyCode = "";
registerEmailForm.addEventListener('submit', (e) => {
  e.preventDefault();
  const emailInputBtn = registerEmailForm.querySelector('button');
  emailInputBtn.disabled = true; // Prevent multiple clicks
  emailInputBtn.innerText = currentLang === 'en' ? "Sending..." : "እየላከ ነው...";
  ...
  // በ EmailJS አዲስ አካውንት ሲከፍቱ የሚሰጠዎትን Service ID እና Template ID እዚህ ጋር ይተኩ
  emailjs.send("YOUR_SERVICE_ID", "YOUR_TEMPLATE_ID", {
    to_email: userProfile.email,
    verification_code: expectedVerifyCode,
  }).then(function () {
    emailInputBtn.disabled = false;
    emailInputBtn.innerText = currentLang === 'en' ? "Send Code" : "ኮድ ላክ";
    authStepRegister.classList.add('hidden');
    ...
    emailInputBtn.disabled = false;
    emailInputBtn.innerText = currentLang === 'en' ? "Send Code" : "ኮድ ላክ";
    alert(currentLang === 'en' ? "Failed to send verification email. Please check your network." : "ኮዱን መላክ አልተቻለም። ኢንተርኔትዎን ይፈትሹ።");
  });
});
...
completeProfileForm.addEventListener('submit', async (e) => {
  ...
  if (pwd.length !== 4 || !/^\d{4}$/.test(pwd)) {
    alert(currentLang === 'en' ? "Password must be exactly 4 digits." : "የይለፍ ቃል 4 ቁጥሮች ብቻ መሆን አለበት።");
    return;
  }
  if (pwd !== pwdConfirm) {
    alert(currentLang === 'en' ? "Passwords do not match." : "የይለፍ ቃሎቹ አይመሳሰሉም።");
    return;
  }
  ...
  if (!userProfile.location || !userProfile.niche) {
    alert(currentLang === 'en' ? 'Please select location and niche.' : 'እባክዎ ቦታ እና የስራ ዘርፍ ይምረጡ!');
    return;
  }
...
logoutBtn.addEventListener('click', () => {
  isLoggedIn = false;
  userProfile = { email: '', fullName: '', businessName: '', location: '', niche: '', phone: '', password: '', avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Felix', activePackage: null, payments: [], apexId: '' };
  updateProfileUI();
  goToHome();
});
...
btnOpenHistory.addEventListener('click', () => {
  ...
  if (userProfile.payments && userProfile.payments.length > 0) {
    userProfile.payments.forEach(p => {
      historyList.innerHTML += `<div style="padding: 15px; border: 1px solid var(--glass-border); border-radius: 10px; background: rgba(0,0,0,0.1); margin-bottom: 10px;"><strong>${p.package}</strong><br><span style="color: var(--accent);">${p.amount} ETB</span> - Status: ${p.status}</div>`;
    });
  } else {
...
document.getElementById('settings-form').addEventListener('submit', async (e) => {
  ...
  if (oldPwd || newPwd) {
    ...
    if (hashedOldPwd !== userProfile.password) {
      alert(currentLang === 'en' ? "Old password is incorrect!" : "የቀድሞው የይለፍ ቃል ትክክል አይደለም!");
      return;
    }
    if (newPwd.length !== 4 || !/^\d{4}$/.test(newPwd)) {
      alert(currentLang === 'en' ? "New Password must be 4 digits!" : "አዲሱ የይለፍ ቃል 4 ቁጥሮች መሆን አለበት!");
      return;
    }
  ...
  try {
    ...
    alert(currentLang === 'en' ? "Settings Updated!" : "ማስተካከያው ተቀይሯል!");
  } catch (err) {
    console.error(err);
  }
});
  ...
  btn.addEventListener('click', async function () {
    ...
    try {
      await addDoc(collection(db, "support_chats"), {
        Customer: userProfile.fullName, "Issue Type": msgVal, Time: dateStr, Status: "Unread", Email: userProfile.email
      });
    } catch (e) {
      console.error(e);
    }
...
// Universal Toggles
aboutNavBtn.addEventListener('click', () => { aboutModal.classList.remove('hidden'); });
closeAboutBtn.addEventListener('click', () => { aboutModal.classList.add('hidden'); });
faqNavBtn.addEventListener('click', () => { faqModal.classList.remove('hidden'); });
...
downloadPdfBtn.addEventListener('click', () => {
  ...
  const opt = {
    margin: 10, filename: `${userProfile.apexId}_Contract.pdf`, image: { type: 'jpeg', quality: 0.98 },
...
// Chapa Integration (ተስተካክሎ ያለችግር Inline እንዲከፍት የተሰራ)
async function initiateChapaPayment(amount, description, daysDuration) {
  ...
  if (typeof ChapaCheckout === "undefined") {
    alert(currentLang === 'en' ? "Chapa is not loaded. Please refresh the page." : "ቻፓ አልተጫነም። እባክዎ ገፁን አድሰው ይሞክሩ (Refresh)።");
    return;
  }

  // የ Chapa Public Key (Test key ነው፥ ለትክክለኛ ክፍያ በ Live key ይቀይሩት)
  ...
  try {
    let checkout = new ChapaCheckout({
      ...
      onPaymentFailure: function () {
        alert(currentLang === 'en' ? "Payment Failed. Please try again." : "ክፍያዎ አልተሳካም። እባክዎ እንደገና ይሞክሩ።");
      },
      customization: {
